...
namespace Core.Services.Stadiums
{
    public class StadiumService : IStadiumService
    {
        ...
        public async Task UpdateStadium(UpdateStadiumDto updatedStadium)
        {
            ...
            Stadium? stadium = _repository.Stadium.Find(e => e.Id == updatedStadium.Id).FirstOrDefault();
            if (stadium == null)
            {
                throw new KeyNotFoundException($"Stadium with ID {updatedStadium.Id} was not found.");
            }

            bool cityExists = _repository.City.Find(e => e.Id == updatedStadium.CityId).Any();
            if (!cityExists)
            {
        ...
        {
            IQueryable<Stadium> stadiums = _repository.Stadium.GetAllAsync();
            List<StadiumDTO> stadiumDto = stadiums.Select(e => new StadiumDTO
            {
                Id = e.Id,
                Name = e.Name,
                CityId = e.CityId,
